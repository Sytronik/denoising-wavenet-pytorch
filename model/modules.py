"""
https://github.com/r9y9/wavenet_vocoder/tree/master/wavenet_vocoder
"""
...
def get_wnorm_conv1d(in_channels, out_channels, kernel_size, dropout=0, std_mul=4.0, **kwargs):
    m = CustomConv1d(in_channels, out_channels, kernel_size, **kwargs)
...
def get_conv1d3x1(in_channels, out_channels, bias=True, weight_normalization=True):
    """1-by-1 convolution layer
    """
    if weight_normalization:
        assert bias
        return get_wnorm_conv1d(in_channels, out_channels, kernel_size=3, padding=1,
                                dilation=1, bias=bias, std_mul=1.0)
    else:
        return CustomConv1d(in_channels, out_channels, kernel_size=3, padding=1,
                            dilation=1, bias=bias)


def get_conv1d1x1(in_channels, out_channels, bias=True, weight_normalization=True):
    """1-by-1 convolution layer
    """
    if weight_normalization:
        assert bias
        return get_wnorm_conv1d(in_channels, out_channels, kernel_size=1, padding=0,
                                dilation=1, bias=bias, std_mul=1.0)
    else:
        return CustomConv1d(in_channels, out_channels, kernel_size=1, padding=0,
                            dilation=1, bias=bias)
...
class ResidualConv1dGLU(nn.Module):
    ...
    def __init__(self, residual_channels, gate_channels, kernel_size,
        ...
        if weight_normalization:
            assert bias
            self.conv = get_wnorm_conv1d(residual_channels, gate_channels, kernel_size,
                                         padding=padding, dilation=dilation,
                                         bias=bias, std_mul=1.0, *args, **kwargs)
        else:
            self.conv = CustomConv1d(residual_channels, gate_channels, kernel_size,
                                     padding=padding, dilation=dilation,
                                     bias=bias, *args, **kwargs)

        # local conditioning
        if cin_channels > 0:
            self.conv1x1c = get_conv1d1x1(cin_channels, gate_channels,
                                          bias=bias,
        ...
        # global conditioning
        if gin_channels > 0:
            self.conv1x1g = get_conv1d1x1(gin_channels, gate_channels, bias=bias,
                                          weight_normalization=weight_normalization)
        else:
            self.conv1x1g = None
        ...
        self.conv1x1_out = get_conv1d1x1(gate_out_channels, residual_channels, bias=bias,
                                         weight_normalization=weight_normalization)
        self.conv1x1_skip = get_conv1d1x1(gate_out_channels, skip_out_channels, bias=bias,
                                          weight_normalization=weight_normalization)
