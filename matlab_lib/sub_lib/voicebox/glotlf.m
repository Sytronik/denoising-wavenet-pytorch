...
%      Version: $Id: glotlf.m 10204 2017-10-06 07:27:41Z dmb $
%
...
if ~nargout
    ...
    texthvc(q.ti,0,' ti','lbk');
    texthvc(q.tp,0,' tp','lbk');
    if d==0
        plot(xlim,[1;1]*q.Up,':k');
        texthvc(0,q.Up,' Up','ltk');
        texthvc(te,0,' te','lbk');
    elseif d==1
        plot(xlim,[1;1]*[q.Ei -q.Ee],':k');
        plot([te te+q.ta te+q.ta],[-q.Ee 0 ylim(2)],':k');
        texthvc(te,0,' te','rtk');
        texthvc(te+q.ta,0,' te+ta','lbk');
        texthvc(0,-q.Ee,' –Ee','lbk');
        texthvc(0,q.Ei,' Ei','ltk');
    elseif d==2
        plot(xlim,[1;1]*q.Ee/q.ta,':k');
        texthvc(0,q.Ee/q.ta,' Ee/ta','ltk');
        texthvc(te,0,' te','lbk');
    end
    hold off
