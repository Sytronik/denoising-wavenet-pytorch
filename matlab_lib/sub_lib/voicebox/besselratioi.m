...
%      Version: $Id: besselratioi.m 10395 2018-02-22 17:00:45Z dmb $
%
...
if any(mn)              % we need to do one or two Newton iterations
    ...
    smn=smn+(besselratio(smn,0,p+1)-rmn).*ymn;              % do a Newton iteration
    mr=(rmn>=0.75) & (rmn<=0.875);                          % need a second Newton iteration for 0.75<=r<=0.875
    if any(mr)
        smn(mr)=smn(mr)+(besselratio(smn(mr),0,p+1)-rmn(mr)).*ymn(mr); % do a second Newton iteration
    end
